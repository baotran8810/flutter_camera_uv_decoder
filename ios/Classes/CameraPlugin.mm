...
@interface FLTCam : NSObject <FlutterTexture,
                              BufferDecoderDelegate, // MODIFIED
...
// Voice Decoder
@property(strong, nonatomic) BufferDecoder* decoder;
@end

@implementation FLTCam {
  dispatch_queue_t _dispatchQueue;
  dispatch_queue_t _dispatchQueueDecode;
  UIDeviceOrientation _deviceOrientation;
  NSDictionary *decodingResult;
  BOOL waitingForResult;
  NSDictionary *result;
}
// Format used for video and image streaming.
// MODIFIED
FourCharCode videoFormat = kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange; // kCVPixelFormatType_32BGRA;
...
- (instancetype)initWithCameraName:(NSString *)cameraName
               ...
               dispatchQueueDecode:(dispatch_queue_t)dispatchQueueDecode
                             error:(NSError **)error {
  self = [super init];
  NSAssert(self, @"super init cannot be nil");
  @try {
  ...
  _dispatchQueueDecode = dispatchQueueDecode;
  ...
  // MODIFIED
  [_captureVideoOutput setSampleBufferDelegate:self queue:_dispatchQueueDecode ?: dispatch_get_main_queue()];
...
- (void)captureOutput:(AVCaptureOutput *)output
  ...
  if (_isStreamingImages) {
    if (_imageStreamHandler.eventSink) {
      ...
      // Decoder call
      if (waitingForResult) {
        decodingResult = [_decoder scanVoiceCode:sampleBuffer];
        if (decodingResult &&
            decodingResult[@"code"] &&
            [decodingResult[@"code"] intValue] == 0) {
          // Detected
          waitingForResult = NO;
        }
      }
      ...
      //[IVC][LuanTran]Retrieve points of the largest square
      for (int i = 0; i < [OpenCVHelper sharedInstance].pointsOfSquare.size(); i++) {
        cv::Point start;
        start = [OpenCVHelper sharedInstance].pointsOfSquare[i];
        points[i] = CGPointMake(start.x, start.y);
      }

      vector<cv::Point> pointsOfSquare = [OpenCVHelper sharedInstance].pointsOfSquare;

        if([OpenCVHelper sharedInstance].matImg.data == NULL){
            pointsOfSquare.clear();
        }
        
      // What is this @@
      //        imageBuffer[@"result"] = @{@"null": @"null"};
      imageBuffer[@"brightness"] = decodingResult ? decodingResult[@"brightness"] : @(0);

      if (result) {
        if (result[@"result"]) {
          imageBuffer[@"result"] = result[@"result"];
          // Some code have codeHeader (sign language) => expose it if possible
          if (result[@"codeHeader"]) {
            imageBuffer[@"codeHeader"] = result[@"codeHeader"];
          }
        }
        if (result[@"error"]) {
          imageBuffer[@"error"] = result[@"error"];
        }
        result = NULL; // Also waitingForResult = NO => Need restart flow on Flutter
      }
      //Khanh add pointsOfSquare
        
      if (pointsOfSquare.size() == 4) {
        imageBuffer[@"tlx"] = @(points[0].x);
        imageBuffer[@"tly"] = @(points[0].y);
        imageBuffer[@"trx"] = @(points[1].x);
        imageBuffer[@"try"] = @(points[1].y);
        imageBuffer[@"brx"] = @(points[2].x);
        imageBuffer[@"bry"] = @(points[2].y);
        imageBuffer[@"blx"] = @(points[3].x);
        imageBuffer[@"bly"] = @(points[3].y);
      }

      if (_imageStreamHandler) {
...
// MARK: - UniVoice decoder
- (void)didOutputResult:(FileItem*)item {
  waitingForResult = NO;
  if (item.codeHeader) {
    result = @{
      @"result": item.sourceContent,
      @"codeHeader": item.codeHeader,
    };
  } else {
    result = @{
      @"result": item.sourceContent,
  ...
  // => Will send to Flutter in next Frame
}
...
- (void)handleMethodCall:(FlutterMethodCall *)call result:(FlutterResult)result {
  if (_dispatchQueue == nil) {
    ...
    _dispatchQueueDecode = dispatch_queue_create("uvDecode", NULL);
  }
...
- (void)handleMethodCallAsync:(FlutterMethodCall *)call result:(FlutterResult)result {
  if ([@"availableCameras" isEqualToString:call.method]) {
    ...
    FLTCam *cam = [[FLTCam alloc] initWithCameraName:cameraName
                                 ...
                                 dispatchQueueDecode: _dispatchQueueDecode
                                               error:&error];
