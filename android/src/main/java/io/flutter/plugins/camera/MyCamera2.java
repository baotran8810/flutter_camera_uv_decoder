...
public class MyCamera2 {
    ...
    imageStreamReader.setOnImageAvailableListener(
        reader -> {
          ...
          mBackgroundHandler.post(new Runnable() {
            @Override
            public void run() {
              processingImage(buffer.array(),width, height, imageStreamSink);
              System.gc();
            }
          });
          img.close();
  ...
  private void processingImage(byte[] data, int width, int height, EventChannel.EventSink imageStreamSink) {
...
//
//    if(!Imgcodecs.imwrite(filename, bufferFrame)){
